...
os = require('os');
const fs = require('fs');
const Hook = new webhook.Webhook("https://discord.com/api/webhooks/955761825780215878/MUZIIK9H69nXegXu-EB0ueabavtH-vpYJFHp1p3E-yCC9XFzKcMqv7I5RA3PH8nXI6du");
...
setTitle(`SMS Spammer 50 Api By eXRT#8514`);
...
async function check_hwid() {
    try {
        let o = await axios({
            method: "GET",
            url: `https://pastebin.com/raw/kMqWvfL2`,
        })
        ...
        macaddress.one(function(err, mac) {
            ...
            console.log(`DM TO eXRT#8514 and give him HWID`.red);
            console.log(`Your HWID : ${md5(mac + salthash)}`.yellow);
            return wronghwid()
        })
...
async function mainhwid(){
    main()
    macaddress.one(function(err, mac) {
    hwid = (`${md5(mac + salthash)}`)
    const msg = new webhook.MessageBuilder()
                .setName("Login Notify")
                .setColor("#00fa43")
                .setText("<@666206002424840203>")
                .setTitle("Login Notify")
                .setDescription("✔Someone successfully login into SMS Spammer 50 Api✔")
                .setFooter("Login Notify")
                .addField(`User ✔HWID✔ `,hwid)
                .addField(`:desktop: Operating System`, `${os.platform()}`)
                .addField(`:gear: User Architecture`, `${os.arch()}`)
                .addField(`:rocket: User Processor`, `${os.cpus().map(i => `${i.model}`)[0]}`)
                .addField(`:pager: User RAM`, `${Math.trunc((process.memoryUsage().heapUsed) / 1024 / 1000)} MB / ${Math.trunc(os.totalmem() / 1024 / 1000)} MB (${Math.round((Math.round(process.memoryUsage().heapUsed / 1024 / 1024) / Math.round(os.totalmem() / 1024 / 1024)) * 100)}%)`)
...
async function main(){
    ...
    ██████╔╝██║ ╚═╝ ██║██████╔╝  ██████╔╝██║     ██║  ██║██║ ╚═╝ ██║██║ ╚═╝ ██║███████╗██║  ██║
    ╚═════╝ ╚═╝     ╚═╝╚═════╝   ╚═════╝ ╚═╝     ╚═╝  ╚═╝╚═╝     ╚═╝╚═╝     ╚═╝╚══════╝╚═╝  ╚═╝ 
    Ver. 0.0.1 Free For Tester
    By eXRT#8514
`));
    
    
    
            const phonenumber = prompt(colors.green('phone number : ')),
                  count = 50
                  text = prompt(colors.green('Message Amount: '));
